                        ...
                        foreach ($this->fltr as $filtercommand) {
                                ...
                                switch ($command) {
                                        ...
                                        case 'wmi': // WaterMarkImage
                                                ...
                                                @list($filename, $alignment, $opacity, $size, $margin['x'], $margin['y'], $rotate_angle) = explode('|', $parameter, 7);
                                                ...
                                                if ( @is_readable($filename) )
                                                {
                                                        if ( $img_watermark = $this->ImageCreateFromFilename($filename) )
                                                        {
                                                                ...
                                                                {
                                                                        // Get the image width.
                                                                        $img_width = ImageSX($this->gdimg_output);
                                                                                
                                                                        // Get the image height.
                                                                        $img_height = ImageSY($this->gdimg_output);
                                                                                
                                                                        // Get the watermark width.
                                                                        $watermark_width = ImageSX($img_watermark);
                                                                                
                                                                        // Get the watermark height.
                                                                        $watermark_height = ImageSY($img_watermark);
                                                                        
                                                                        if ($watermark_width > $watermark_height)
                                                                        {
                                                                                // Calculate the watermark's new width.
                                                                                $watermark_new_width = $img_width * $size / 100;
                                                                                        
                                                                                // Calculate the watermark's new height.
                                                                                $watermark_new_height = $watermark_height * $watermark_new_width / $watermark_width;
                                                                        }
                                                                        else
                                                                        {
                                                                                // Calculate the watermak's new height.
                                                                                $watermark_new_height = $img_height * $size / 100;
                                                                                        
                                                                                // Calculate the watermark's new width.
                                                                                $watermark_new_width = $watermark_width * $watermark_new_height / $watermark_height;
                                                                        }
                                                                                
                                                                        if ( $watermark = phpthumb_functions::ImageCreateFunction($watermark_new_width, $watermark_new_height) )
                                                                        {
                                                                                ImageAlphaBlending($watermark, false);
                                                                                ImageSaveAlpha($watermark, true);
                                                                                        
                                                                                $this->ImageResizeFunction( $watermark, $img_watermark, 0, 0, 0, 0, ImageSX($watermark), ImageSY($watermark), ImageSX($img_watermark), ImageSY($img_watermark) );
                                                                                $img_watermark = $watermark;
                                                                        }
                                                                        else
                                                                        {
                                                                                $this->DebugMessage( 'ImageCreateFunction('. $watermark_width . ', ' . $watermark_height . ') failed', __FILE__, __LINE__ );
                                                                        }        
                                                                        ...
                                                                        if ( preg_match('#^([0-9\\.\\-]*)x([0-9\\.\\-]*)$#i', $alignment, $matches) )
                                                                        {
                                                                                ...
                                                                                if ($scale != 1)
                                                                                {
                                                                                        ...
                                                                                        {
                                                                                                ImageAlphaBlending($img_watermark2, true);
