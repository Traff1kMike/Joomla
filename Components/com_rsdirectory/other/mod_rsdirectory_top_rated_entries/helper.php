...
define('MOD_RSDIRECTORY_TOP_RATED_ENTRIES_VERSION', '1.0.2');

/**
 * RSDirectory! Top Rated Entries Module Helper.
 */
abstract class RSDirectoryTopRatedEntriesHelper
{
    /**
    ...
    public static function getEntries($params)
    {
		...
		$order = array(
			$db->qn('e.avg_rating') . ' DESC',
			$db->qn('e.ratings_count') . ' DESC',
			$db->qn('e.promoted') . ' DESC',
		);
